...
// Manager is an about:blank popup that reads/writes via postMessage.
...
!function(){
  ...
  window.addEventListener("message", function(e){
    ...
    // Manager asked for current scripts
    if(d.type === "dt_fetch"){
      try{
    ...
    // Manager saved a new script list — persist it here
    if(d.type === "dt_save"){
      saveScripts(d.scripts);
    }
    // Manager wants to run code on this page right now
    if(d.type === "dt_run"){
      try{ Function(d.code)(); }
  ...
  function openManager(){
    ...
    doc.close();
    // Give manager time to set up listener then push scripts
    setTimeout(function(){
      try{
        if(_managerWin && !_managerWin.closed)
          _managerWin.postMessage({ type:"dt_load", scripts:loadScripts() }, "*");
  ...
  function buildManagerHTML(openerHost){
    ...
    // Ask opener for scripts on load (handles case where setTimeout fired before listener was ready)
    +'try{'
    +  'if(window.opener&&!window.opener.closed)'
    +    'window.opener.postMessage({type:"dt_fetch"},"*");'
    +'}catch(e){}'

    // Write back to opener + persist
    +'function persist(){'
    +  'try{'
    +    'if(window.opener&&!window.opener.closed){'
