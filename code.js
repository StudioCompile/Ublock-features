...
!function(){
  ...
  // ── Bridge ───────────────────────────────────────────────────────
  // KEY INSIGHT: browsers block postMessage to cross-origin tabs.
  // The opener (google.com) CANNOT send to the target tab.
  // But the target tab CAN send to window.opener (google.com) freely.
  //
  // Protocol (all initiated by the BRIDGE TAB):
  //   1. Bridge tab loads, detects it's a bridge via window.name
  //   2. Bridge tab posts { type:"uf_bridge_ready", name:window.name } to window.opener
  //   3. Opener receives "uf_bridge_ready", looks up pending push by name, responds
  //      by storing scripts directly via the shared localStorage IF same origin...
  //      but we're cross-origin, so we can't.
  //
  // REAL solution: store the payload in the OPENER's sessionStorage keyed by window.name.
  // The bridge tab reads it directly from window.opener.sessionStorage — but that's
  // also cross-origin blocked.
  //
  // ONLY WORKING APPROACH without a server:
  //   Use the URL itself to pass a small token, then the bridge tab reads its OWN
  //   localStorage (which is what we want anyway), receiving the payload via
  //   postMessage from opener to the tab — but we said that's blocked...
  //
  // The ONE exception: postMessage TO an iframe IS allowed cross-origin.
  // So: open the target origin in a HIDDEN IFRAME (not a tab), then postMessage to it.
  // The iframe has uFeatures injected (via userscript — userscripts run in iframes too
  // if configured with @match, which Violentmonkey/Tampermonkey do by default).
  //
  // This is the correct and working approach. No tab opened at all.
  ...
  function pushToSite(origin, scripts, onDone){
    ...
    // Create hidden iframe pointing to target origin
    iframe = document.createElement("iframe");
    iframe.style.cssText = "position:fixed;width:1px;height:1px;bottom:-10px;left:-10px;opacity:0;pointer-events:none;border:none";
    iframe.src = origin + "/?__ufb=1";

    msgHandler = function(e){
      var d = e.data; if(!d || typeof d !== "object") return;

      // Bridge iframe signals it loaded and is ready
      if(d.type === "uf_bridge_ready" && e.source === iframe.contentWindow){
        // Now we CAN postMessage to an iframe we own — this is allowed cross-origin
        try{
          iframe.contentWindow.postMessage({
            type: "uf_bridge_set",
            key:  SITE_KEY,
            scripts: scripts
          }, origin); // use exact origin as target for security
        }catch(ex){ finish("send-failed: "+ex); }
      }

      // Bridge iframe saved successfully
      if(d.type === "uf_bridge_ack" && e.source === iframe.contentWindow){
    ...
    timeoutId = setTimeout(function(){
      finish("timeout — is uFeatures injected on "+origin+"?");
    }, 8000);
  }

  // ── Bridge iframe listener (runs on the target site) ─────────────
  // The hidden iframe on the target site receives messages from the opener.
  window.addEventListener("message", function(e){
    var d = e.data; if(!d || typeof d !== "object") return;

    // Opener checks if the iframe is ready — we respond immediately on load
    // (handled in the takeover IIFE below)

    // Receive scripts to save
    if(d.type === "uf_bridge_set" && d.key && Array.isArray(d.scripts)){
      try{
        localStorage.setItem(d.key, JSON.stringify(d.scripts));
        try{ e.source.postMessage({ type:"uf_bridge_ack" }, e.origin); }catch(ex){}
      }catch(ex){
  ...
  // ── Bridge iframe takeover ────────────────────────────────────────
  // Detects if this page is loaded as a bridge iframe (via ?__ufb=1 OR window.name).
  // Signals ready to the parent frame immediately.
  (function(){
    var isBridge = location.search.indexOf("__ufb=1") !== -1
                || (window.name && window.name.indexOf("uf_bridge_") === 0);
    // Must be in an iframe context, not the top window, and not the settings page
    var inIframe = window !== window.top;
    if(!isBridge || !inIframe || IS_SETTINGS) return;

    // Signal parent we are ready as soon as possible
    function signalReady(){
      try{ window.parent.postMessage({ type:"uf_bridge_ready" }, "*"); }catch(ex){}
    }
